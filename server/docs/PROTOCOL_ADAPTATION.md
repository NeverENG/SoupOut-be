...
- `serverTick` = 地盘变更所属 tick：增量帧在 sim 推进**前**发出（game.go:95），取 `t = r.g.Tick`（game.go:191）——帧在 `Step()` 前发出，serverTick = 内容截至的 tick，与 keyframe 及 ACK 契约一致；`sinceTick` = 该玩家最近回传的 ACK（`r.terrSince[p]`，game.go:212）。
